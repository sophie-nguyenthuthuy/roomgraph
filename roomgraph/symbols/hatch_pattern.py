...
def _families(group: list[Seg]) -> list[tuple[float, float, int]]:
    """(angle, spacing, count) for each family of parallel rulings."""
    ...
    for family in buckets:
        ...
        out.append((round(angle, 1), round(spacing, 1), len(family)))
    return sorted(out)


def _signature(families) -> tuple[str, list[dict]]:
    described = [
        {"angle_deg": a, "spacing_mm": sp, "rulings": n} for a, sp, n in families
    ]
    if len(families) == 1:
...
def detect(ctx: PlanContext) -> Match | None:
    ...
    swatches = legend_swatches(ctx)
    specimens: list[tuple[list[tuple[float, float, int]], str]] = []
    swatch_boxes: list[tuple[float, float, float, float]] = []
    for centre, size, caption in swatches:
        half = size * 0.75
        box = (centre.x - half, centre.y - half, centre.x + half, centre.y + half)
        swatch_boxes.append(box)
        inside = [
            s for s in rulings
